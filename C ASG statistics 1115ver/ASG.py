...
#         self.isSleepSink = False
    ...
    def gettimeofday(self):
        ...
        if not self.isTimeSink:
            args  = self.syscall['args'].split(",")
            timestamp = int(args[0][1:].split("=")[1])
    #         print(timestamp)
    ...
    def shmget(self):
        ...
        info["shm_id"] = "Shared Memory ID:" + shm_id
    ...
    def shmat(self):
        ...
        info["shm_id"] = "Shared Memory ID:" + shm_id # 從 ID 指向 addr
        if self.isMemSink:
...
#             "setsid",
    ...
    def _connect_node(self, from_node, to_node, edge):
        ...
        if to_node.name != "Unknown" and from_node.name != "Unknown": 
...
#                 print("Chdir:", info["path"])
                if info["path"].strip("\"")[-1] != "/":
